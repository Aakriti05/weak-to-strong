    ...
    with torch.no_grad():
        ...
        for E in range(0, rounds):
            ...
            def maybe_load_model(model):
                if (
                    os.path.exists(os.path.join(save_path, "results.pkl"))
            ...
            if already_trained:
                model.load_state_dict(
                    torch.load(os.path.join(save_path, "pytorch_model.bin"))
                )
        ...
        for i in tqdm(train2_ds):
            ...
            for m in range(E + 1):
                ...
                labels = np.argmax(labels.to(dtype=torch.float32), axis=-1)
                logits = model(input_ids)
                pred_individual_model = np.argmax(
                ...
                if pred_individual_model == labels:
                    model_acc[m] += 1
                probs += models[m][1] * torch.nn.functional.softmax(logits, dim=-1).to(
                    "cpu"
                )
            preds = np.argmax(probs.to(dtype=torch.float32), axis=-1)
